...
module.exports=async(context,req)=>{
 ...
 try{
  ...
  if(req.method==='GET'){
   const reports={};try{for await(const item of container.listBlobsFlat({prefix:'reports/'})){const saved=JSON.parse((await container.getBlockBlobClient(item.name).downloadToBuffer()).toString());if(saved.removed)continue;const {sourceText,...report}=saved;reports[saved.agency+'|'+saved.group+'|'+saved.kind]=report;}}catch(e){if(e.statusCode!==404)throw e;}
  ...
  if(req.method==='DELETE'){
   ...
   try{const saved=JSON.parse((await blob.downloadToBuffer()).toString());await blob.uploadData(Buffer.from(JSON.stringify({...saved,removed:true,removedAt:new Date().toISOString()})),{blobHTTPHeaders:{blobContentType:'application/json'}});}catch(e){if(e.statusCode!==404)throw e;}

...
module.exports=async(context,req)=>{
 ...
 try{
  const container=BlobServiceClient.fromConnectionString(process.env.MASTER_STORAGE_CONNECTION_STRING).getContainerClient('esd-dts-master');const fy=String(req.query?.fy||'FY26').toUpperCase();if(!/^FY(2[5-9]|30)$/.test(fy))throw Error('Invalid fiscal year.');const blob=container.getBlockBlobClient('loa-reference-'+fy+'.json');
  if(req.method==='GET'){try{return reply(200,JSON.parse((await blob.downloadToBuffer()).toString()));}catch(e){if(e.statusCode!==404)throw e;if(fy==='FY26'){try{return reply(200,JSON.parse((await container.getBlockBlobClient('loa-reference.json').downloadToBuffer()).toString()));}catch(legacyError){if(legacyError.statusCode!==404)throw legacyError;}}return reply(200,{mapping:(await import('./loa-map.mjs')).default,organizations:{},updatedAt:null});}}
  ...
  await container.createIfNotExists();await container.getBlockBlobClient('loa-source-'+fy+'.xlsx').uploadData(Buffer.from(sourceBase64,'base64'),{blobHTTPHeaders:{blobContentType:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'}});
  const value={mapping:clean,organizations:cleanOrganizations,updatedAt:new Date().toISOString(),fiscalYear:fy};await blob.uploadData(Buffer.from(JSON.stringify(value)),{blobHTTPHeaders:{blobContentType:'application/json'}});return reply(200,value);
